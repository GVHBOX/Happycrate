...
**范围是核心源码**：`app/`、`web/`、`main.py`、`tests/`。
这里不写注释、不写 docstring，名字起清楚就行。
`tools/` 是辅助开发与生产的工具，不算核心源码 —— 那里的 docstring / 注释照常写，别去"清理"它。

例外只有两种：工具指令（`# noqa` 这类）、被运行时真正读取的字符串。
整理存量代码时，已有注释属于清除对象（只清核心源码里的）。
...
**不要往 `.workbuddy/` 里写任何东西，也不要在项目里重建它。** 那是宿主（WorkBuddy）
强制新建的目录，项目不用它（`.gitignore` 里已忽略）。

**不要写跨会话的记忆/日志文件。** 项目不需要这种文件：会执行的规矩写在 `AGENTS.md`
（自动注入）与 `tools/README.md`（有指针指过去），这两处才是权威。往磁盘上另记一份
只会跟它们漂移，而且下一个会话未必会读。
